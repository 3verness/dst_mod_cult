...
local lambtent_assets =
{
    Asset("ANIM", "anim/lambtent.zip"),
	Asset("ATLAS", "images/inventoryimages/lambtent.xml"),	
	Asset("IMAGE", "images/inventoryimages/lambtent.tex"),	
}

-----------------------------------------------------------------------
--For regular lambtents
...
local function onbuilt_lambtent(inst)
    inst.AnimState:PlayAnimation("setting")
    inst.AnimState:PushAnimation("idle", true)
...
local function lambtent()
    local inst = common_fn("lambtent", "lambtent", "tent.png", nil, onbuilt_lambtent)

    if not TheWorld.ismastersim then
        return inst
    end

    inst.sleep_anim = "sleeping"
...
return Prefab("lambtent", lambtent, lambtent_assets),
    MakePlacer("lambtent_placer", "lambtent", "lambtent", "idle")
